...
///use for verify the user
const verifyToken = (req, res, next) => {
    const token = req?.cookies?.token
    // console.log('token in the middleware', token)

    ///if dont have token then use this
    if (!token) {
...
async function run() {
    try {
        ...
        app.post('/jwt', logger, async (req, res) => {
            ...
            ///set cookie
            //when user login set cookie into local storage cookie in application
        ...
        //// when user log out remove the cookie for safety
        app.post('/logout', async (req, res) => {

            const user = req.body;

            //for seeing user log out
            console.log('logging out', user);

            ///clear the cookie after log out
            ///maxAge : 0 no need to save into the localstorage
